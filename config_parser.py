...
class VisaConfig:
    ...
    def __init__(self, config_file: str):
        ...
        for embassy_section in [section for section in config.sections() if section.count('EMBASSY') > 0]:
            ...
            embassy_config = EmbassyConfig(embassy_code, period_start, period_end, should_reschedule)

            if 'BLOCKED_DAYS' in config[embassy_section]:
                embassy_config.blocked_days = [date_str.strip() for date_str in config[embassy_section]['BLOCKED_DAYS'].split(',')]
            self.embassies.append(embassy_config)
